@vsk-ingest
Feature: Ingestion of the VSK Access Type Dimensions 
...
Scenario Outline: Uploading the dimension files to buckets 
    ...
    * if (responseStatus == 200 )  karate.call('ingest_vsk_dimensions.feature@valid')
    * if (responseStatus == 400) karate.call('ingest_vsk_dimensions.feature@invalid')
    * if (responseStatus == 500 || responseStatus == 501 || responseStatus == 502 || responseStatus == 503 || responseStatus == 504 ) karate.fail('Server error')
        ...
        Examples:
        |file|
        |academicyear-dimension.data.csv|
        ...
        |schoolcategory-dimension.data.csv|
        |schoolmanagement-dimension.data.csv|
