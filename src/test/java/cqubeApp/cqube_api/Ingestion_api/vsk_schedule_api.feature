...
Scenario: Schedule School Attendance Processor
    And path '/api/spec/schedule'
    And request {"processor_group_name": "school_attendance_"+ storage_type ,"scheduled_at": "0 "+(min+6+" ")+(hour+"")+" * * ?"}
    When method Post 
    Then match responseStatus == 200
    And match response.message == "Successfully updated the schedule"
...
# If Data Migration is enabled then comment below scenarios     
